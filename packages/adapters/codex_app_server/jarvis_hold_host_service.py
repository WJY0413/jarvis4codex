    ...
    try:
        if _matching_health(state_dir, profile, codex_home, **health_checks):
            if _matching_health(state_dir, profile, codex_home, required_workers=workers, **health_checks):
                return {"status": "ready", "phase": "already_running"}
            return {"status": "failed", "reason": "HoldHost worker capacity is insufficient"}
        # Reserve idle execution slots only when creating a Host; never resize a live one.
        workers = max(10, int(workers))
...
def _start_hold_host(*, state_dir: Path, launcher_config: Path, workers: int, poll_seconds: float) -> subprocess.Popen[str]:
    ...
    if os.name == "nt":
        ...
        host_args = [
            ...
            "--poll-seconds", str(max(poll_seconds, 0.25)), "--workers", str(max(workers, 10)),
        ]
...
class JarvisHoldHost:
    ...
    def __init__(self, *, state_dir: Path, launcher_config: Path, workers: int = 1) -> None:
        ...
        self.workers = max(int(workers), 10)
...
def main() -> int:
    ...
    parser.add_argument("--workers", type=int, default=1, help="requested capacity; newly created Hosts reserve at least 10 idle slots")
