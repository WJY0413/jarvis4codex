...
class TaskProvisioningAdapterContractTest(unittest.TestCase):
    ...
    def test_create_queues_for_the_user_host_without_starting_an_app_server(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            saved = json.loads((Path(temp) / "task-holds" / "hold-create-1" / "request.json").read_text(encoding="utf-8"))
            self.assertEqual(saved["source_ref"], "mcp:test")
    ...
    def test_resume_archives_the_previous_terminal_receipt_before_queueing(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(json.loads((root / "request.json").read_text(encoding="utf-8"))["source_ref"], "mcp:test")
