...
class ReliabilityRegressionTest(unittest.TestCase):
    def test_same_hold_resume_cannot_skip_an_unverified_bound_candidate(self):
        ...
        for case in ("missing", "omitted_binding", "forged_summary", "valid", "schema_invalid_output", "schema_forged_summary", "schema_resume_omit", "schema_resume_downgrade"):
            with self.subTest(case=case), tempfile.TemporaryDirectory() as temp:
                ...
                self.assertEqual(before["status"], "turn_limit_reached" if case in {"valid", "schema_resume_omit", "schema_resume_downgrade"} else "blocked")
    ...
    def test_windows_detached_host_uses_space_paths_and_drains_a_stopped_test_hold(self):
        ...
        with tempfile.TemporaryDirectory(prefix="Jarvis Loop Test ' $ ") as temp:
            ...
            try:
                ...
                self.assertEqual(health["worker_capacity"], 10)
    ...
    def test_saved_output_gate_covers_wrong_missing_old_and_last_candidate(self):
        ...
        for case in cases:
            with self.subTest(case=case), tempfile.TemporaryDirectory() as temp:
                ...
                self.assertEqual(final["status"], "turn_limit_reached" if valid else "blocked")
                self.assertEqual(len(client.started_turns), 1 if valid or case in {"last_missing", "schema_last_bad"} else 0)
                self.assertEqual(final["total_turn_count"], 2 if valid or case in {"last_missing", "schema_last_bad"} else 1)
                self.assertEqual(final["output_verification"]["status"], "verified" if valid else "blocked")
    ...
    def test_last_candidate_missing_saved_output_is_blocked(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            request, ack, result = (root / name for name in ("request.json", "ack.json", "result.json"))
            ...
            with patch("adapters.codex_app_server.jarvis_task_hold_host.NativeTaskLauncherConfig", return_value=FakeConfig()), patch(
                ...
                self.assertEqual(hold_task(root / "launcher.json", request, ack, result), 0)
            self.assertEqual(json.loads(result.read_text(encoding="utf-8"))["status"], "blocked")
    ...
    def test_initialize_user_host_does_not_start_a_second_live_host_for_more_capacity(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            (state_dir / "hold-host.json").write_text(json.dumps({
                "status": "holding", "active_count": 1, "pid": 1780, "worker_capacity": 1,
...
class HostCapacityRegressionTest(unittest.TestCase):
    ...
    def test_new_host_minimum_applies_to_initializer_constructor_and_process_entry(self):
        from adapters.codex_app_server.jarvis_hold_host_service import _start_hold_host
        for requested, expected in ((1, 10), (3, 10), (12, 12)):
    ...
    def test_two_loops_share_new_ten_slot_host_without_extra_tasks(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch("adapters.codex_app_server.jarvis_hold_host_service._start_hold_host", side_effect=start) as starter, patch(
                ...
                try:
                    self.assertEqual(adapter.ensure_hold_host_ready(required_workers=3)["status"], "ready")
...
class BatchLaneRegressionTest(unittest.TestCase):
    ...
    def _run_batch(self, count, size, *, lanes=1, case="valid", with_schema=True):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            for lane in range(lanes):
                ...
                boundary.mkdir()
                threads.append({"slot": f"worker-{lane}", "lane": {
            ...
            for child, spec in zip(started.data["children"], threads):
                ...
                def emit(_thread_id, turn_id):
                    ...
                    self.assertEqual(ids, spec["lane"]["candidate_ids"][(turn-1)*size:turn*size])
                ...
                expected = "turn_limit_reached" if case == "valid" else "cancelled" if case == "stop" else "blocked"
                self.assertEqual(final["status"], expected)
                self.assertEqual(final["total_turn_count"], rounds if case in {"valid", "last_schema"} else 1)
            ...
            self.assertEqual(result.status, "completed" if case == "valid" else "stopped" if case == "stop" else "blocked")
            if case == "valid":
